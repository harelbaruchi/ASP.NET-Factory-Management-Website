...
namespace FactoryWebSite.Models
{
    public class EmployeeBL
    {
        ...
        {
            var empToUpdate = db.employees.Where(x => x.ID == emp.ID).FirstOrDefault();
        ...
        public List<employee> SearchEmp(string srchTerm)
        {
            var emp = GetEmployees();
            return emp.Where(x => x.FirstName.Contains(srchTerm) || x.LastName.Contains(srchTerm)).ToList();
        } 

        public void AddShiftToEmp(shift shf,employee emp)
        {
            //var shiftID = shf.ID;
            //var empID = emp.ID;
